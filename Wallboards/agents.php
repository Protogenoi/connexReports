...
WHERE
    live_agents.campaign_id IN (1001, 2001, 2002)
    ...
    if ($query->rowCount() > 0) {
        while ($result = $query->fetch(PDO::FETCH_ASSOC)) {
            ...
            <tr class="<?php echo $class; ?>">
                <td><strong><span <?php if (substr($Time, -5) >= 7 && $status == 'INCALL') {
                            echo "class='blinking'";
                        } ?>><?php echo $result['full_name']; ?></span></strong></td>
                <td><span <?php if (substr($Time, -5) >= 7 && $status == 'INCALL') {
                        echo "class='blinking'";
                    } ?>><?php echo $status; ?></span></td>
                <td><span <?php if (substr($Time, -5) >= 7 && $status == 'INCALL') {
                        echo "class='blinking'";
                    } ?>><?php echo substr($Time, -5); ?></span></td>
            </tr>
