...
while ($i < $group_ct) {
	$stmt = "SELECT status, status_name, human_answered, sale, dnc, customer_contact, not_interested, unworkable, scheduled_callback, completed from campaign_status where campaign_id='$group[$i]' UNION SELECT status, status_name, human_answered, sale, dnc, customer_contact, not_interested, unworkable, scheduled_callback, completed from vicidial_statuses order by status, status_name";
	$rslt = mysql_to_mysqli($stmt, $link);
	...
	while ($row = mysqli_fetch_row($rslt)) {
		...
		if (mysqli_num_rows($stat_rslt) > 0) {
			...
			$ASCII_text .= "+------------------------------------------+--------+------------+-------------+\n";
			$ASCII_text .= "| " . _QXZ("DISPOSITION", 40) . " | " . _QXZ("CALLS", 6) . " | " . _QXZ("DURATION",
					10) . " | " . _QXZ("HANDLE TIME", 11) . " |\n";
			$ASCII_text .= "+------------------------------------------+--------+------------+-------------+\n";
			...
			while (list($key, $val) = each($dispo_ary)) {
				...
				$CSV_text .= "\"" . $key . $status_ary[$key] . "\",\"$val[0]\",\"" . sec_convert($val[1],
						'H') . "\",\"" . sec_convert($val[2], 'H') . "\"\n";
			...
			$ASCII_text .= "+------------------------------------------+--------+------------+-------------+\n";
			$CSV_text .= "\"" . _QXZ("TOTALS") . ":\",\"$total_calls\",\"" . sec_convert($total_duration,
					'H') . "\",\"" . sec_convert($total_handle_time, 'H') . "\"\n\n";
			...
			$ASCII_text .= "| " . _QXZ("TOTAL CALLS", 14,
					"r") . ": $header_list_count                                   |\n";
			$ASCII_text .= "+--------------------------------------------------------------+\n";
			$ASCII_text .= "| " . _QXZ("STATUS FLAGS BREAKDOWN",
					22) . ":  " . _QXZ("(and % of total leads in the list)", 34) . "  |\n";
			...
			$graph_id++;
			$graph_array = array(
				"CSL_CALLS" . $list_id . "data|1|CALLS|integer|",
				"CSL_DURATION" . $list_id . "data|2|DURATION|time|",
				"CSL_HANDLETIME" . $list_id . "data|3|HANDLE TIME|time|"
			);
			...
			for ($q = 0; $q < count($graph_array); $q++) {
				...
				switch ($dataset_type) {
					case "time":
						$graph_totals_array[$q] = "  <caption align=\"bottom\">" . _QXZ("TOTAL") . " - " . sec_convert($current_graph_total,
								'H') . " </caption>\n";
